...
package Foswiki::Plugins::VotePlugin::Core;
...
sub handleVOTE {
  ...
  my $class = $Foswiki::cfg{VotePlugin}{Modules}{$type};
  return _inlineError("unknown voting type '$type'") unless defined $class;

  my $id = defined($params->{id}) ? $params->{id} : '_default';
  my $isGlobal = Foswiki::Func::isTrue($params->{global}, 0);
  my $isOpen = Foswiki::Func::isTrue($params->{open}, 1);
  my $isSecret = Foswiki::Func::isTrue($params->{secret}, 1);
  my $bayesian = Foswiki::Func::isTrue($params->{bayesian}, 0);
  my $submit = Foswiki::Func::isTrue($params->{submit}, 1);
  my $saveto = $params->{saveto};

  my @prompts = ();

  my $defaultStarsFormat = '| $key$reset | $small<div class="star-info">Score: $score, My vote: $mylast, Total votes: $sum</div> |';
  ...
  if (defined($params->{select})) {
    push(
      ...
      {
        type => 'select',
        name => Foswiki::Func::decodeFormatTokens($params->{select}),
        format => $defaultSelectFormat,
        options => [
          map { Foswiki::Func::decodeFormatTokens($_) }
  ...
  while (1) {
    if (defined($params->{"select$n"})) {
      push(
        ...
        {
          type => 'select',
          name => Foswiki::Func::decodeFormatTokens($params->{"select$n"}),
      ...
      unless (($params->{"width$n"} || 5) =~ /^\d+$/) {
        return _inlineError("Expected integer width for stars$n=");
      }
      push(
        @prompts,
        {
  ...
  # check attributes
  if (!scalar(@prompts)) {
    return _inlineError("no prompts specified " . $params->stringify());
  }
  ...
  if ($this->canReset($id)) {
    ...
    if (defined $query->param('Reset' . $id)) {
      $this->clearVotesData($web, $topic, $id, $isGlobal, $saveto);
      $mess = "$id has been reset";
    }
  }

  # read in the votes
  my $lines = $this->getVoteData($web, $topic, $id, $isGlobal, $saveto);
  my %votes;
  ...
  my $act;
  if ($isOpen) {
    $act = Foswiki::Func::getScriptUrl($web, $topic, 'view');
  } else {
    $act = Foswiki::Func::getScriptUrl($web, $topic, 'viewauth');
  }
  ...
  if ($submit) {
    ...
    while (my ($k, $v) = each %hidden) {
      $hiddens .= CGI::input({type => 'hidden', name => $k, value => $v});
    }
  ...
  if ($this->canReset($id)) {
    if ($mess) {
      ...
      $mess =~ s/\r?\n//g;    # CGI::start_form adds unwanted \n's
      $mess .= CGI::submit({name => 'Reset' . $id, value => 'Reset'}) . CGI::end_form();
    }
  ...
  # so we can embed votes in tables
...
sub canReset {
  ...
  $this->{_canReset} ||= {};
  my $canReset = $this->{_canReset}{$id};

  return $canReset if defined $canReset;
  $canReset = 0;

  my $controls = Foswiki::Func::getPreferencesValue('VOTEPLUGIN_RESETTERS');
  if (defined $controls) {
...
sub registerVote {
  ...
  _writeDebug("called registerVote($id)");
  ...
  my $ident = $this->getIdent($isSecret, $isOpen);

  #    $ident = int(rand(100))
  #      if $debug; # for testing

  # Apply a weighting for the voting user
  my $weightsTopic = Foswiki::Func::getPreferencesValue('VOTEPLUGIN_WEIGHTINGS');
  my $weight = 1;
  if ($weightsTopic) {
    my ($wweb, $wtopic) = Foswiki::Func::normalizeWebTopicName($web, $weightsTopic);
    if (Foswiki::Func::topicExists($wweb, $wtopic)) {
      my ($meta, $text) = Foswiki::Func::readTopic($wweb, $wtopic);
      foreach my $line (split(/\n/, $text)) {
        if ($line =~ /^\|\s*(\S+)\s*\|\s*(\d+)\s*\|$/) {
          ($wweb, $wtopic) = Foswiki::Func::normalizeWebTopicName(undef, $1);
  ...
  $this->saveVotesData($web, $topic, $id, $query->param('vote_isGlobal') || 0, $query->param('vote_saveTo') || '', $voteData);

  # invalidate cache entry
  if (defined &Foswiki::Cache::invalidateEntry) {
    Foswiki::Cache::invalidateEntry($web, $topic);
  }
}

sub saveVotesData {
  my ($this, $web, $topic, $id, $isGlobal, $saveto, $voteData) = @_;
  if ($saveto) {
    my $text = '';
    my $meta = '';
    $saveto =~ /(.*)/;
    my ($vw, $vt) = Foswiki::Func::normalizeWebTopicName($web, $1);
    if (Foswiki::Func::topicExists($vw, $vt)) {
      ($meta, $text) = Foswiki::Func::readTopic($vw, $vt);
    }
    $text .= $voteData;
    Foswiki::Func::saveTopic($vw, $vt, $meta, $text, {minor => 1});
  } else {
    my $votesFile = $this->getVotesFile($web, $topic, $id, $isGlobal);
...
sub clearVotesData {
  my ($this, $web, $topic, $id, $isGlobal, $saveto) = @_;
  if ($saveto) {
    my $text = '';
    my $meta = '';
    $saveto =~ /(.*)/;
    ...
    $text =~ s/(^|\n)((\|.*\||\s+)\n+)*$/$1/s;
    Foswiki::Func::saveTopicText($vw, $vt, $meta, $text, {minor => 1});
  } else {
    my $votesFile = $this->getVotesFile($web, $topic, $id, $isGlobal);
...
sub getVoteData {
  ...
  my $lines = '';
  if ($saveto) {
    my ($vw, $vt) = Foswiki::Func::normalizeWebTopicName($web, $saveto);
    if (Foswiki::Func::topicExists($vw, $vt)) {
      my $meta;
      ($meta, $lines) = Foswiki::Func::readTopic($vw, $vt);
    }
  } else {
    my $votesFile = $this->getVotesFile($web, $topic, $id, $isGlobal);
    if (open(F, "<$votesFile")) {
...
sub getVotesFile {
  my ($this, $web, $topic, $id, $global) = @_;

  my $path = Foswiki::Func::getWorkArea('VotePlugin');
  my $flatweb = $web;
  $flatweb =~ s/\//./g;
  my $votesFile = $path . '/' . ($global ? '' : "${flatweb}_${topic}_") . ($id ? "_$id" : '');
  $votesFile = _normalizeFileName($votesFile);

  if (!-e $votesFile) {
    my $attachPath = Foswiki::Func::getPubDir() . "/$web/$topic";
...
sub showSelect {
  my ($this, $id, $prompt, $submit, $select, $keyValueFreq, $totalVotes, $params) = @_;
  ...
  my $bars = '';
  foreach my $value (
    sort { $keyValueFreq->{$b} <=> $keyValueFreq->{$a} }
    keys %{$keyValueFreq}
    )
  {
    ...
    my $bar = Foswiki::Func::decodeFormatTokens($prompt->{chart});
    $bar =~ s/\$option/$value/;
    $bar =~ s/\$perc/$perc/g;
    $bar =~ s/\$score/$score/g;
...
sub _makeBar {
  ...
  my $pubUrlPath = Foswiki::Func::getPubUrlPath() . '/' . $Foswiki::cfg{SystemWebName} . '/VotePlugin';
  my $graph = CGI::img({
...
sub showLineOfStars {
  my ($this, $form, $prompt, $submit, $needSubmit, $act, $mean, $myLast, $total) = @_;

  my $max = $prompt->{width};
  my $perc = $total ? int(1000 * $mean / $total) / 10 : 0;

  $mean = sprintf("%.3g", $mean);
  my $row = Foswiki::Func::decodeFormatTokens($prompt->{format});
  ...
  if ($submit && !$needSubmit) {
    $eAttrs = {onChange => "javascript: document.getElementById('$form').submit()"};
  }

  my $ul = Foswiki::Contrib::RatingContrib::renderRating('vote_data_' . $prompt->{name}, $max, ($row =~ /\$small/) ? 1 : 0, $mean, $eAttrs);
